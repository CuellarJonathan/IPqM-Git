...
export default async function TuboPage() {
  const { data: items, error } = await supabase
    .from('tubos')
    .select('*')
    .order('numero_serie_tubo', { ascending: false })
              ...
              {items?.map((item) => (
                <tr key={item.numero_serie_tubo}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <Link href="/tubos/${item.numero_serie_tubo}" className="text-blue-600 hover:text-blue-900 mr-4">
                      Editar
                    </Link>
